...
<div class="row py-2" style="padding-bottom:0px !important">
    <div class="col align-self-center" style="width:80%">
        <h2>
            @if($stockreport_data->srtype == 'stockin')
                Stock In
            @elseif($stockreport_data->srtype == 'stockout')
                Stock Out
            @elseif($stockreport_data->srtype == 'returned')
                Returned
            @elseif($stockreport_data->srtype == 'received')
                Received
            @elseif($stockreport_data->srtype == 'voided')
                Voided
            @elseif($stockreport_data->srtype == 'transfer')
                Transfer
            @elseif($stockreport_data->srtype == 'stocktake')
                Stock Take
            @elseif($stockreport_data->srtype == 'cforward')
                Carry Forward
            @elseif($stockreport_data->srtype == 'refundcp')
                Refund Cp
            @elseif($stockreport_data->srtype == 'daily_variance')
                Daily Variance
            @endif
        </h2>
    </div>
...
<table class="table table-bordered" id="prodstockreport" style="width:100%;">
    ...
    <tr>
        ...
        <th style="text-align: center; width: 80px;">@if($stockreport_data->srtype == 'stockin') Qty In @else Qty Out @endif</th>
    </tr>
    </thead>
    <tbody>
