...
/* #turn_on  #turn_off :hover{
    border: 2px solid #3edeca;
} */
</style>
@endsection
@section('content')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-start mt-2 mb-4">
        <div class="mr-3">
            <button type="button" id="turn_on" style="width: 70px !important" class="btn btn-prawn float-right" onclick="auto_stockin_activate()">  On</button>
        </div>
        <div class="">
            <button type="button" id="turn_off" style="width: 70px !important;" class="btn btn-prawn float-right" onclick="auto_stockin_deactivate()"> Off</button>
        </div>
...
let autostockin = {{ $company->auto_stockin ?? 0 }};
...
if(autostockin == 0){
    ...
    $("#turn_on").css({'border' : '1px #aaaaaa solid'})
    $("#turn_on").css('color', '#aaaaaa')
}else{
    $("#turn_off").css({'border' : '1px gray solid'})
    $("#turn_off").css('color', 'gray')
}
...
async function auto_stockin_activate() {
	await $.ajax({
		url: "{{route('screend.setting.autostockin.turn_on')}}",
		type: 'get',
		success: function (response) {
            $("#turn_off").prop('disabled', false);
             $("#turn_off").css({'border' : '1px #aaaaaa solid'})
            $("#turn_off").css('color', '#aaaaaa')
			messageModal("Auto Stock In turned on successfully.");

            $("#turn_on").prop('disabled', true);
            $("#turn_on").css({'border' : '2px  #3edeca solid'})
            $("#turn_on").css('color', '#3edeca')
		},
		error: function (e) {
			console.log('Error:'+e);
		}
...
async function auto_stockin_deactivate() {
	await $.ajax({
		url: "{{route('screend.setting.autostockin.turn_off')}}",
		type: 'get',
		success: function (response) {
            $("#turn_on").prop('disabled', false);
            $("#turn_on").css({'border' : '1px #aaaaaa solid'})
            $("#turn_on").css('color', '#aaaaaa')
			messageModal("Auto Stock in turned off successfully.");
            $("#turn_off").prop('disabled', true);
            $("#turn_off").css({'border' : '2px #3edeca solid'})
            $("#turn_off").css('color', '#3edeca')
