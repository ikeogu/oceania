		...
		<div class="row"
			...
			<div class="col-md-4 pl-0 align-self-center" style="">
             ...
             <div class="col-md-3 pl-0 align-self-center" style="">
				<p style="margin-bottom: 0; font-size:18px; font-weight:bold;"> {{ $location->name }}</p>
                <p style="margin-bottom: 0;font-size:18px;font-weight:bold;"> {{ $location->systemid }}</p>
                <p style="margin-bottom: 0;font-size:18px;font-weight:bold;"> Doc No. {{ $docId }}</p>
			</div>
            <div class="col-md-3 pl-0 align-self-center" style="">
				<p style="margin-bottom: 0;font-size:18px;font-weight:bold;"> {{ $user->fullname }}</p>
                <p style="margin-bottom: 0;font-size:18px;font-weight:bold;"> {{ $user->systemid }}</p>
                <p style="margin-bottom: 0;font-size:18px;font-weight:bold;"> {{ $time}}</p>
			</div>
...
var tbl_url = "{{route('receiving_notes.get_datatable_products')}}";
var tableData = {
	systemid: "{{ Request::route('id') }}"
};
...
var rec_note_table = $('#rec_note_table').DataTable({
    ...
    "ajax": {
        ...
        "url": "{{route('receiving_notes.confirmed_datatable')}}",
        "type": "POST",
        data: function (d) {
...
function downLoadExcel(){
    ...
    $.ajax({
		url: "{{route('receiving.excel_download')}}",
