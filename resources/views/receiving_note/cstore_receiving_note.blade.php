...
async function search_product_barcode(product_barcode) {
        ...
        // get the product id
        await $.ajax({
            url : "{{ route('receiving_notes.get_product_id') }}",
            type: "POST",
            data : FormData,
	...
	tbl_url = "{{ route('receiving_notes.search_barcode') }}";
...
function update_quantity() {
	...
	let dataLength = "{{ count($receivingnotes) + 1 }}";
	let container = [];
	let product = {};
	...
	if (stockin_updates.length > 0) {
		...
		$.ajax({
			url: "{{route('update_receive_notes')}}",
			type: "POST",
				'headers': {
...
var rec_note_table = $('#rec_note_table').DataTable({
   ...
   "ajax": {
        ...
        "url": "{{route('receiving_notes.datatable')}}",
        "type": "POST",
        data: function (d) {
