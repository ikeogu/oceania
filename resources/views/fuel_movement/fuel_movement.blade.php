	...
	<div class="d-flex mt-0" style="width: 100%;">
		...
		<div class="col-sm-3 pl-0">
			<div class="row mb-0" style="float:right;">
				<button onclick="window.open('{{ route("fuel.guide") }}')"
					class="btn btn-success btn-fuel-guide sellerbutton"
					style="padding:0px;margin-right:5px"
					id="guide_btn">Guide
				</button>

				<form action="{{route('fuel_movement.pdf')}}"
					style="margin-bottom:0!important"
					method="post" id="PrintPDF">
					@csrf
				...
				<button onclick="window.open('{{ route("fuel.stockIn") }}')"
				...
				<button onclick="window.open('{{ route("fuel.stockOut") }}')"
...
$('#c_ForwardEditModal-').on('hidden.bs.modal', function (e) {
	...
	$.ajax({
		url: "{{route('fuel_movement.cforward.update')}}",
		async: true,
		type: 'POST',
		'headers': {
			'X-CSRF-TOKEN': '{{ csrf_token() }}',
...
$('#tank_dipEditModal-').on('hidden.bs.modal', function (e) {
	...
	$.ajax({
		url: "{{route('fuel_movement.tankdip.update')}}",
		type: 'POST',
		async: true,
		'headers': {
			'X-CSRF-TOKEN': '{{ csrf_token() }}',
...
function showOgFuelProducts() {
	$.ajax({
		url: "{{route('fuel_movement.showOgFuelProducts')}}",
		async: false,
		type: "GET",
		dataType: "JSON",
		success: function (response) {
...
function getMainTable() {
	...
	$.ajax({
		url: "{{route('fuel_movement.fuelmovementmaintable')}}",
		async: false,
		type: 'POST',
		'headers': {
			'X-CSRF-TOKEN': '{{ csrf_token() }}',
...
var companycreated = "{{$company->approved_at??date('Y-m-d')}}";
