...
function escpos_print_template() {
	...
	'{{!empty($company->office_address)? "(".$company->office_address.")" : ""}}' + '\x0A',
	((addy1 != '') ? addy1 : '') + '\x0A',
	((addy2 != '') ? addy2 : '') + '\x0A',
...
//   '123456789012345678901234567890123456789012345'
    ...
    'Rounding                 ' +'{{str_pad(number_format(($eoddetailsdata->rounding/100),2), 20, " ", STR_PAD_LEFT)}}' + '\x0A',
...
//    '123456789012345678901234567890123456789012345'
	...
	'Cash                               '+ '{{str_pad(number_format(((($eoddetailsdata->cash) ??"0.00")/100),2), 10, " ", STR_PAD_LEFT)}}' + '\x0A',

    'Credit Card                        ' +'{{str_pad(number_format(((($eoddetailsdata->creditcard)??"0.00")/100),2), 10, " ", STR_PAD_LEFT)}}' + '\x0A',


	'Wallet                             ' +'{{str_pad(number_format(((($eoddetailsdata->wallet)??"0.00")/100),2), 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'Credit Account                     ' +'{{str_pad(number_format(((($eoddetailsdata->creditac)??"0.00")/100),2), 10, " ", STR_PAD_LEFT)}}' + '\x0A',

    'Outdoor Payment Terminal ' +'{{str_pad(empty($opos_eoddetails->opt) ? "0.00":number_format(($opos_eoddetails->opt/100),2), 20, " ", STR_PAD_LEFT)}}' + '\x0A',

    'Outdoor Electronic Wallet ' +'{{str_pad(empty($opos_eoddetails->oew) ? "0.00":number_format(($opos_eoddetails->oew/100),2), 19, " ", STR_PAD_LEFT)}}' + '\x0A',
