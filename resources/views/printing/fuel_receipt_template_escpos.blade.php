...
//	'123456789012345678901234567890123456789012345'
	...
	'Credit Card              '+ @if ($receipt->payment_type == "creditcard")'{{str_pad(!empty($receiptdetails->creditcard)?number_format((($receiptdetails->creditcard/100)+((5 * round(($receiptdetails->creditcard-$receipt->cash_change) / 5))-($receiptdetails->creditcard-$receipt->cash_change))/100),2):"0", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',

//	'123456789012345678901234567890123456789012345'
	'Wallet                   '+ @if ($receipt->payment_type == "wallet")'{{str_pad(!empty($receiptdetails->wallet)?number_format((($receiptdetails->wallet/100)+((5 * round(($receiptdetails->wallet-$receipt->cash_change) / 5))-($receiptdetails->wallet-$receipt->cash_change))/100),2):"0.00", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',

//	'123456789012345678901234567890123456789012345'
	'Credit Account           '+ @if ($receipt->payment_type == "creditac")'{{str_pad(!empty($receiptdetails->creditac)?number_format((($receiptdetails->creditac/100)+((5 * round(($receiptdetails->creditac-$receipt->cash_change) / 5))-($receiptdetails->creditac-$receipt->cash_change))/100),2):"0.00", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',
	...
	'\x1B' + '\x61' + '\x31',	// center align
	'Thank You!' + '\x0A',
	'This is the final receipt. ' + '\x0A',
	'Should there be a refund,' + '\x0A',
	'the calculation below is applicable.' + '\x0A',
