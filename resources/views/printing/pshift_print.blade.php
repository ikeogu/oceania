...
function escpos_print_template() {
	...
	'\x0A',
    '\x1B' + '\x21' + '\x08',	// emphasized mode on
    'Personal Shift Summary       '+ '{{$login_time}}' + '\x0A',
    '                             '+ '{{$logout_time}}'+ '\x0A',
	'---------------------------------------------' + '\x0A',
...
//    '123456789012345678901234567890123456789012345'
	...
	'Credit Account                     ' +'{{str_pad($creditac, 10, " ", STR_PAD_LEFT)}}' + '\x0A',
...
//    '123456789012345678901234567890123456789012345'
	...
	'Non-Operational Cash In            '+ '{{str_pad($non_op_cash_in, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'(-)Non-Operational Cash Out        ' +'{{str_pad($non_op_cash_out, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'(-)Sales Drop                      ' +'{{str_pad($sales_drop, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'---------------------------------------------' + '\x0A',
//    '123456789012345678901234567890123456789012345'
	'\x1B' + '\x21' + '\x00',	// emphasized mode off

	'Expected Amount                    '+ '{{str_pad($expected_amount, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'Actual Drawer Amount               ' +'{{str_pad($actual_drawer_amount, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'---------------------------------------------' + '\x0A',
//    '123456789012345678901234567890123456789012345'
	'\x1B' + '\x21' + '\x00',	// emphasized mode off

	'Difference                         ' +'{{str_pad($difference, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'---------------------------------------------' + '\x0A',
//    '123456789012345678901234567890123456789012345'
	'\x1B' + '\x21' + '\x00',	// emphasized mode off

	'C-Store Total                      ' +'{{str_pad($cstore_total, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	// We need 1 tabstop: 35th
	'\x1B' + '\x44' + '\x23' + '\x00',
	@if(!empty($fuel_prd))
		@foreach ($fuel_prd as $prd)
//  '123456789012345678901234567890123456789012345'
    '\x1B' + '\x21' + '\x00',	// emphasized mode off
	'{{(strlen($prd->name) > 33) ? substr($prd->name,0,33) : str_pad($prd->name, 33, " ", STR_PAD_RIGHT) }}' + '\x09' +

    '{{str_pad($prd->sales, 10, " ", STR_PAD_LEFT)}}' + '\x0A',
