...
function escpos_print_template() {
    ...
    @foreach ($receiptproduct as $i)
	 ...
	 @if ($receipt->payment_type == "wallet")
	 	console.log('**THE VALUEwallet**')
	 	console.log('{{$receiptdetails->wallet}}')
	 @endif
	@endforeach
...
//	'123456789012345678901234567890123456789012345'
	...
	@foreach ($receiptproduct as $i)
        ...
        '{{str_pad(number_format(($i->quantity*($i->price/100)) - ($i->discount / 100),2), 7, " ", STR_PAD_LEFT)}}' + '\x0A',
...
//	'123456789012345678901234567890123456789012345'
	...
	'Credit Card              '+ @if ($receipt->payment_type == "creditcard")'{{str_pad(!empty($receiptdetails->creditcard)?number_format(($receiptdetails->creditcard/100),2):"0", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',

	'Wallet                   '+ @if ($receipt->payment_type == "wallet")'{{str_pad(!empty($receiptdetails->wallet)?number_format(($receiptdetails->wallet/100),2):"0", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',
