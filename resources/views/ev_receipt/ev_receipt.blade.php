  ...
  width:92%; font-weight:normal !important;" />
    ...
    <div class="row align-items-center" style="font-weight: normal">
        ...
        @if($receipt->hours != 0)
            {{$receipt->hours}}
        @else
        {{number_format($receipt->kwh / 100, 2)}}
        @endif
        </div>
        <div class="pl-3 col-md-2 receipt-item-c text-center" style="padding-left: 25px !important">
            <span class="global_currency">
                {{ number_format($receipt->rate / 100, 2) }}
            </span>
        </div>
        <div class="col-md-1 receipt-item-discount" style="padding-left: 22px !important">
        @if(isset($receiptdetails->discount))
            {{number_format($receiptdetails->discount / 100, 2)}}
        @else
            0.00
        @endif
        </div>
        <div class="col-md-3 receipt-item-r" style="padding:0px; margin-left:-22px;">
            <span id="item_amount">
            @if($receipt->hours != 0)
                {{ number_format($receipt->rate / 100*$receipt->hours,2) }}
            @else
                {{number_format(($receipt->rate / 100) * ($receipt->kwh / 100), 2)}}
            @endif
            </span>
        </div>
    </div>
   ...
   @if ($receipt->status == 'voided') block;
        ...
        <div class="col-md-6" style="text-align: right;">
            <span style="font-weight:normal" id="item_amount">
                @if ($receipt->payment_type == 'creditcard')
                    {{ !empty($receiptdetails->creditcard) ? number_format($receiptdetails->creditcard / 100 + (5 * round(($receiptdetails->creditcard - $receipt->cash_change) / 5) - ($receiptdetails->creditcard - $receipt->cash_change)) / 100, 2) : '0.00' }}
                @else 
                    0.00
                @endif
            </span>
    ...
    <div class="row" style="text-align: center;margin: 10px auto;">
     ...
     font-weight:bold;color:black;font-size:40px;">
                    {{ !empty($refund->lot_no) ? $refund->lot_no : '' }}
                </span>
            </button>
    ...
    float: left;margin-bottom: 5px; border-radius: 10px;">
        ...
        <div class="col-md-7 pr-4 d-flex" style="justify-content:space-around">

            <button class="btn btn-success bg-receipt-print" style="padding-left:0;padding-right:0; margin-right: 5px;
    font-size:13px;border-radius:10px; " onclick="print_receipt({{ $receipt->id }})">
                <strong>Print</strong>
            </button>

            <!-- <button class="btn btn-success btn-log bg-receipt-void
    sellerbutton void_true only_void_true only_void" style="padding-left:0; padding-right:0;  margin-right: 0;
        font-size:13px;border-radius:10px;
      @if ($receipt->status === 'voided' || $receipt->status === 'refunded') background: #3a3535;
                cursor:not-allowed; @endif"
    ...
    'voided') block @else none @endif" >
        ...
        <div class="row">
            <div class="col-md-6 text-left" style="color:red;">
                <strong> {{ $receipt->staff_user->fullname ?? '' }}</strong>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 text-left" style="color:red;">
                <strong> {{ $receipt->staff_user->systemid ?? '' }}</strong>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 text-left" style="color:red;">
...
<script type="text/javascript">
    ...
    window.addEventListener('storage', (e) => {
        switch (e.key) {
            case "ev_receipt_voided":
                console.log("voided: ", $('.rec_id').attr('id'))
                if (localStorage.getItem('ev_receipt_voided1') === $('.rec_id').attr('id')) {
                    $.ajax({
                        method: "post",
                        url: "{{ route('ev_receipt.evReceiptDetail') }}",
                        data: {
                            id: localStorage.getItem('ev_receipt_voided1')
                        }
                    }).done((data) => {
                        localStorage.removeItem('ev_receipt_voided1');
                        //console.log(data);
                ...
                localStorage.removeItem('ev_receipt_voided')
                break;
        }
    });

    function loyalty(receipt_id) {
        console.log('loyalty()');
    }

    function print_receipt(evreceipt_id) {
        
        console.log('PR print_receipt()');

        $.ajax({
            url: "{{ route('receipt.ev.print') }}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            type: 'post',
            data: {
                evreceipt_id: evreceipt_id,
            },
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.ajax({
                    ...
                    console.log(data);
                    $(".detail_view").html(data);
                    localStorage.setItem('ev_receipt_voided', receipt_id);
                    localStorage.setItem('ev_receipt_voided1', receipt_id);
                    localStorage.removeItem('ev_receipt_voided')
