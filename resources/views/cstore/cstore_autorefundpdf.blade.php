...
<div class="rec_id" style="font-size: 14px; font-weight: bold;" id="{{ $receipt->id }}">
  ...
  <hr style="border:0px !important;padding:3px !important;margin:0px !important;">
   <hr class="solid" style="margin: 0px !important;padding: 0px !important;"><hr style="border:0px !important;padding:3px !important;margin:0px !important;">
    ...
    <tr class="void-stamp" id="void-stamp{{ $receipt->id ?? '' }}" style="margin-left: 15% ;display:
...
<div class="container">
        <div style="font-size:12px;text-align:left; color:orange">
            Refunded By:
            <br>
            {{ $ref->name }}
            <br>
            {{ $ref->systemid }}
            <br>
            {{ date('dMy', strtotime($ref->created_at)) }}
			<br>
			MYR {{(number_format($ref->refund_amount,2))}}
			<br>
			{{$ref->description ??''}}
