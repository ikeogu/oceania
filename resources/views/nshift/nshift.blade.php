	...
	<div class="container-fluid">
		...
		<div class="row py-2 align-items-center"
			...
			<div class="col-md-2 text-right"
				...
				<form action="{{route('export_excel_nshift')}}" method="post">
					@csrf
...
function openShiftModal(id)
{
	...
	var url = "{{ route('local_cabinet.nshift.details') }}";
	$.ajax({
		method: "post",
		url: url,
		data: {nshift_id: id}
...
$('#nShiftModal').on('hidden.bs.modal', function (e) {
    ...
    $.ajax({
        method: "post",
        url: "{{route('local_cabinet.nshift.update')}}",
        data: {
        	id: id,
        	cash_in: cash_in,
			cash_out: cash_out,
