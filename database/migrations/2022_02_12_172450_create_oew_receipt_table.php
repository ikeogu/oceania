...
class CreateOewReceiptTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('oew_receipt', function (Blueprint $table) {
            $table->id();
			...
			* This is stored as percent e.g. 6.5% */
			...
			$table->enum('payment_type',
			  ['cash','creditcard','point','wallet','creditac','oew'])->
			  default('oew');

			$table->integer('terminal_id')->unsigned();
			$table->string('terminal_systemid')->nullable();

			// FK to users.id. The user_id of cashier
			$table->integer('staff_user_id')->unsigned();
			$table->string('staff_user_systemid')->nullable();
			...
			$table->string('company_systemid')->nullable();
			...
			* receipt logo */
			...
			// Transacted type
			$table->enum('transacted',['opt','pos','wallet','oew'])->default('oew');
    ...
    {
        Schema::dropIfExists('oew_receipt');
    }
}
