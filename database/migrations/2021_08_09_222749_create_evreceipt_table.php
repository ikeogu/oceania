...
class CreateEvreceiptTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('evreceipt', function (Blueprint $table) {
            $table->id();
			// Unique ID coming from a DB sequence, "evreceipt_seq"
			$table->string('systemid');

			// For storing hours parked and rate
			$table->integer('hours')->unsigned;
			$table->integer('rate')->unsigned;
			$table->integer('kwh')->unsigned;
			$table->enum('ev_mode',['hour','kwh'])->default('hour');
