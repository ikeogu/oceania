    ...
    */
    public function view(): View
    {
        ...
        $query = "
            ...
            FROM
                returningnote_list rl,
                returningnote rn
            LEFT JOIN product p ON p.id = rn.product_id
            LEFT JOIN productbarcode pb
                ON pb.product_id = p.id AND
                pb.selected = 1 AND
                pb.deleted_at is NULL
            WHERE
                rn.systemid = '".$this->request->doc_no."' AND
                rl.returningnote_systemid = '".$this->request->doc_no."'
            ;
        ";

        $data = DB::select(DB::raw($query));

        return view('excel_export.returning_note_excel', [
            'returningNote' => $data,
