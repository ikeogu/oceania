...
class LocalFuelPriceController extends Controller
{
    ...
    "product_id":95
    "prdcategory_id":154
    "prdsubcategory_id":220
    "ptype":"oilgas"
    "location_id":331
    "brand_id":191
    "company_id":13
    "username":"Alex Tan"
    "product_name":"ron96ha"
    "product_systemid":"1030000001126"
    "userid":84
    "time":"2021-10-13 22:47:57"
	*/
    ...
    public function priceUpdate(Request $request){
        ...
        if($prod == false){
			...
			$prd_ogfuel = DB::table('prd_ogfuel')->
				where('product_id' , $prod->id);
            $pd_id = (clone $prd_ogfuel)->first();
			if($pd_id == false){
				$data = ['product_id'=>$prod->id, 'price'=>$price];
				DB::table('prd_ogfuel')->insertGetId($data);
			}else{
				(clone $prd_ogfuel)->update(['price' => $price]);
			}
			$get_prd_ogfuel = (clone $prd_ogfuel)->pluck('id')->first();
			$check_og_localfuelprice = DB::table('og_localfuelprice')->
			 	where('ogfuel_id' , $get_prd_ogfuel)->first();

			if($check_og_localfuelprice == false){
                DB::table('og_localfuelprice')->insert([
					'ogfuel_id'=>$get_prd_ogfuel,
					'company_id'=>$company_id,
					'location_id'=>$location_id,
					'start'=>$time,
					'price'=>$price,
    ...
    public function productUpdate(Request $request){
        ...
        $contents = file_get_contents($photo_url, false, stream_context_create($arrContextOptions));
