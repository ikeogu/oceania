...
EOD;
    ...
    function fuelLocalPriceModal(Request $request)
    {
        try {
            ...
            return view('local_fuelprice.local_pumpconfig_modal',
				compact('pump', 'local_pts2_protocol',
				'baud', 'nozzleData'));
    ...
    function showFuelLocalPriceDatatable(Request $request)
    {
        ...
        select("product.*", 'og_localfuelprice.ogfuel_id',
            'og_localfuelprice.price','og_localfuelprice.controller_price',
			'prd_ogfuel.id as alt_id', 'og_localfuelprice.user_id',
			'og_localfuelprice.updated_at as u_date',
        ...
        addColumn('product_name', function ($data) {
            ...
            $img = "<img src='$img_src' data-field='inven_pro_name'
				...
				border-radius:5px; margin-right: 8px;object-fit:contain;'>";
...
EOD;
    ...
    public function localFuelPricePushHardware(Request $request)
    {
        try {
			...
			Log::debug('localFuelPricePushHardware: fuelgradesConfig='.
				json_encode($fuelgradesConfig));

            $mypetrolStation = new petrolStation();

            foreach ($controller as $c) {
                $pumpConfig = $this->pumpConfig($c->id);
                $nozzleConfig = $this->nozzleConfig($c->id,
					collect($fuelgradesConfig));
    ...
    public function nozzleConfig($controller_id, $fuelConfig)
    {
        try {
            ...
            $nozzleDetails = DB::table('local_pumpnozzle')->
				get()->groupBy('pump_id');
            ...
            foreach ($nozzleDetails as $byPump) {
                ...
                foreach ($byPump as $nozzle) {
                    ...
                    $nozleFuelIndex = $fuelConfig->
						where('og_f_id', $nozzle->ogfuel_id)[0]['Id'] ?? false;
                ...
                $nozzleConfig[] = [
					"PumpId" => $pumpIndex,
					"FuelGradeIds" => array_values($fuelGradeIdsFormated)
				];
                $pumpIndex++;
            }
