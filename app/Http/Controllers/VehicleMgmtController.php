...
EOD;
    ...
    public function postDataToUpdateDownVehicleData(Request $request){
		...
		if ($validator->fails()) {
            ...
            if($request->input('rfid_no')){
                $data['rfid_no'] = $request->
                input('rfid_no');
            }
			...
			where('systemid' , $request->
			input('systemid'))->get()->first();
            if($lg_vehiclemgmt=== null){

                $data['systemid'] = $request->input('systemid');
