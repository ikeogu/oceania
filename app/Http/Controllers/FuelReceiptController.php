...
{
    public function fetch_latest_receipt_id(Request $request)
    {
        Log::info('CTRL ***** fetch_latest_receipt_id *****');
        Log::info('CTRL fetch_latest_receipt_id: FIX: all()=' .
            json_encode($request->all()));

        $my_pump = $request->my_pump;
		...
		WHERE
			fr.id = frl.fuel_receipt_id AND
			frl.pump_no=$my_pump AND
			frl.filled=0 AND
			fr.voided_at IS NULL AND
			fr.created_at BETWEEN '$today 00:00:00' AND '$today 23:59:59'
		ORDER BY
			fr.created_at DESC
		LIMIT 1
		;";

        Log::info('CTRL fetch_latest_receipt_id: FIX: query=' . $query);

        try {
            $response = DB::select(DB::raw($query));
            if (!empty($response))
                $receipt_id = $response[0]->id;
        } catch (\Exceptions $e) {
            Log::error([
                "msg"   => $e->getMessage(),
                "File"  => $e->getFile(),
        ...
        Log::info('CTRL fetch_latest_receipt_id: FIX: receipt_id=' . $receipt_id);
    ...
    public function CreateFuelList(Request $request)
    {
        ...
        try {
            ...
            $receiptproduct_id = DB::table('fuel_receiptproduct')->
            insertGetId([
            ...
            $data['fuel_itemdetails'] = DB::table('fuel_itemdetails')->
                where('receiptproduct_id', $receiptproduct_id)->
                first();
    ...
    {
        Log::debug("fuelRefund: request" . json_encode($request->all()));
        $price = DB::table('fuel_receiptproduct')->join('fuel_receipt', 'fuel_receipt.id', 'fuel_receiptproduct.receipt_id')->join('product', 'product.id', 'fuel_receiptproduct.product_id')->where('fuel_receipt.status', '!=', 'voided')->where('fuel_receipt.id', $request->id)->select('fuel_receiptproduct.price')->first();

        // var_dump($price->price);exit();
        DB::table('fuel_receipt')->where(["id" => $request->id])->update([
                "status" => 'refunded',
                "updated_at" => now(),
            ]);

        $rec = DB::table('fuel_receipt')->select('fuel_receipt.*', 'fuel_receiptdetails.*')->join(
                'fuel_receiptdetails',
                'fuel_receiptdetails.receipt_id',
                'fuel_receipt.id'
            )->where('fuel_receipt.id', $request->id)->first();
        ...
        DB::table('fuel_receiptlist')->where(["fuel_receipt_id" => $request->id,])->update([
                "status" => 'refunded',
                "refund" => ($request->fuel - $request->filled) * 100,
        ...
        $brancheoddata = DB::table('brancheod')->whereDate('created_at', '=', date(
                'Y-m-d',
    ...
    public function dataTable(Request $request)
    {
        try {
            ...
            $totalRecords = count(DB::select(DB::raw(
                "
                SELECT
                    frl.id,
                    fr.status,
                    frl.fuel_receipt_systemid,
                    frl.total,
                    frl.filled,
                    frl.refund,
                    frl.pump_no,
                    frl.fuel_receipt_id as receipt_id,
                    frl.created_at as created_at
                FROM
                    fuel_receipt fr,
                    fuel_receiptlist frl
                WHERE
                    ...
                    frl.created_at between '$date 00:00:00' and '$date 23:59:59'
                ORDER BY id DESC
                ;
            "
            )));
            ...
            return Datatables::of($data)->setOffset($start)->setRowId(function ($data) {
...
EOD;
                    ...
                    if (
                        $row->status != "refunded" && $refund > 0 && $row->status != "voided"
                        && $row->filled != "0.00"
                    ) {
                        $btn = '<a  href="javascript:void(0)"  onclick="refundMe(' . $row->receipt_id . ', ' .
                            $row->total / 100 . ', ' . $row->filled / 100 . ')" data-row="' . $row->id .
                            '" class="delete"> <img width="25px" src="' .
                            asset("images/bluecrab_50x50.png") . '" alt=""> </a>';
                        return $btn;
                    } else {
                        $btn = '<a  href="javascript:void(0)"  style=" filter:
                         grayscale(100) brightness(1.5); pointer-events: none;cursor: default;"
                         data-row="' . $row->id . '" class="delete">
                         <img width="25px" src="' . asset("images/bluecrab_50x50.png") . '"" alt=""> </a>';
                        return $btn;
                    }
                    ...
                    $systemid = !empty($data->fuel_receipt_systemid) ?
                        '<a href="#" style="text-decoration:none" onclick="getFuelReceiptlist('
                        . $data->receipt_id . ')" > ' . $data->fuel_receipt_systemid . '</a>' :
                        'Receipt ID';
                    return <<<EOD
					$systemid
EOD;
    ...
    {
        try {
            $data = DB::table('fuel_receipt')->join(
                    'fuel_receiptlist',
					...
					fuel_receiptlist.pump_no,
					fuel_receiptlist.fuel_receipt_id AS receipt_id,
					fuel_receiptlist.created_at AS created_at')->whereNull('fuel_receiptlist.deleted_at')->where('fuel_receiptlist.pump_no', 'like', $request->pump_no . '%')->whereDate(
                    'fuel_receiptlist.created_at',
                    date('Y-m-d', strtotime($request->date))
                )->orderBy('fuel_receiptlist.id', 'DESC')->get();
    ...
    public function datatable_search_ptype(Request $request)
    {
        ...
        try {
            $data = DB::table('fuel_receipt')->join(
					...
					fuel_receiptlist.pump_no,
					fuel_receiptlist.fuel_receipt_id AS receipt_id,
					fuel_receiptlist.created_at AS created_at')->whereNull('fuel_receiptlist.deleted_at')->where('fuel_receipt.payment_type', 'like', $request->ptype . '%')->whereDate(
                    'fuel_receiptlist.created_at',
                    date('Y-m-d', strtotime($request->date))
                )->orderBy('fuel_receiptlist.id', 'DESC')->get();
...
EOD;
    ...
    public function voidedReceipt(Request $request)
    {
        try {
            ...
            $brancheoddata = DB::table('brancheod')->whereDate('created_at', '=', date(
                    'Y-m-d',
                    strtotime($fuel_receipt->created_at)
                ))->first();
