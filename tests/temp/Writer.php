...
class Writer
{
    ...
    {
		Log::info('***** PF EX export: start *****');

        $this->open($export);

		Log::info('PF EX export: BEFORE export');

		$c = 0;
        $sheetExports = [$export];
        if ($export instanceof WithMultipleSheets) {
			$c++;
			Log::info('PF EX '.$c.' sheetExports'); 
        ...
        foreach ($sheetExports as $sheetExport) {
			$c++;
			Log::info('----> PF EX '.$c.' BEFORE addNewSheet'); 
            $this->addNewSheet()->export($sheetExport);
			Log::info('----> PF EX '.$c.'  AFTER addNewSheet'); 
        }

		Log::info('***** PF EX export: end *****');

        return $this->write($export,
			$this->temporaryFileFactory->makeLocal(null,
				strtolower($writerType)), $writerType);
    }
    ...
    public function write($export, TemporaryFile $temporaryFile, string $writerType): TemporaryFile
    {
		...
		Log::info('PF EX write: AFTER WriterFactory');

        $writer->save(
            $path = $temporaryFile->getLocalPath()
        );

		Log::info('PF EX write: AFTER save()');
    ...
    {
		Log::info('***** addNewSheet: START *****');
        $ret = new Sheet($this->spreadsheet->createSheet($sheetIndex));
		Log::info('***** addNewSheet: END *****');
